 ...
 */
export const URLBlocker = new class {
	blockURL(_user, _content) {
		if (!hasPermission(_user._id, 'send-url')) {
			const result = _content.msg.toLowerCase().match(
				new RegExp("((?:(http|https|Http|Https|\\_http|\\~http|\\*http|rtsp|Rtsp):\\/\\/(?:(?:[a-zA-Z0-9\\$\\-\\_\\.\\+\\!\\*\\'\\(\\)"
					...
					+ '|(?:dabur|dad|dance|data|date|dating|datsun|day|dclk|dds|de|deal|dealer|deals|degree|delivery|dell|deloitte|delta|democrat|dental|dentist|desi|design|dev|dhl|diamonds|diet|digital|direct|directory|discount|discover|dish|diy|dj|dk|dm|dnp|do|docs|doctor|dog|domains|dot|download|drive|dtv|dubai|duck|dunlop|dupont|durban|dvag|dvr|dz)'
					+ '|(?:earth|eat|ec|eco|edeka|edu|education|ee|eg|email|emerck|energy|engineer|engineering|enterprises|epson|equipment|er|ericsson|erni|es|esq|estate|esurance|et|etisalat|eu|eurovision|eus|events|exchange|expert|exposed|express|extraspace)'
					...
					+ '|(?:ibm|icbc|ice|icu|id|ie|ieee|ifm|ikano|il|im|imamat|imdb|immo|immobilien|in|inc|industries|infiniti|info|ing|ink|institute|insurance|insure|int|intel|international|intuit|investments|io|ipiranga|iq|ir|irish|is|ismaili|ist|istanbul|it|itau|itv|iveco)'
					+ '|(?:jaguar|java|jcb|jcp|je|jeep|jetzt|jewelry|jio|jll|jm|jmp|jnj|jo|jobs|joburg|jot|joy|jp|jpmorgan|jprs|juegos|juniper)'
					...
					+ '|(?:la|lacaixa|lamborghini|lamer|lancaster|lancia|land|landrover|lanxess|lasalle|lat|latino|latrobe|law|lawyer|lb|lc|lds|lease|leclerc|lefrak|legal|lego|lexus|lgbt|li|lidl|life|lifeinsurance|lifestyle|lighting|like|lilly|limited|limo|lincoln|linde|link|lipsy|live|living|lixil|lk|llc|llp|loan|loans|locker|locus|loft|lol|london|lotte|lotto|love|lpl|lplfinancial|lr|ls|lt|ltd|ltda|lu|lundbeck|lupin|luxe|luxury|lv|ly)'
					...
					+ '|(?:va|vacations|vana|vanguard|vc|ve|vegas|ventures|verisign|versicherung|vet|vg|vi|viajes|video|vig|viking|villas|vin|vip|virgin|visa|vision|vistaprint|viva|vivo|vlaanderen|vn|vodka|volkswagen|volvo|vote|voting|voto|voyage|vu|vuelos)'
			...
			if (result && result.length) {
				Notifications.notifyUser(_user._id, 'message', {
					_id: Random.id(),
					rid: _content.rid,
					ts: new Date(),
					msg:
						'Sorry, you are not authorized to share URL/IP Address. Your message has been sent without links.',
				});
				result.forEach((url) => {
					_content.msg = _content.msg.replace(
						new RegExp(escapeRegExp(url), 'gi'),
						'~link removed~',
					);
				});
				// return false; //Other option is to send false, and comment forEach, so non of the messages with url, will be sent
			}
		}
		return true;
	}
